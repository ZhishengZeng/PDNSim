	    ...
	    doublecomplex **nzval, int **rowind, int **colptr)
{
     ...
     */
     ...
     /* 1/ read header */ 
     if(fgets(line,512,fp)) printf("Error reading file");
...
#ifdef _LONGINT
	if(fscanf(fp, "%lld%lld%lf%lf\n", &row[nz], &col[nz], &val[nz].r, &val[nz].i)) printf("Error reading file");
#else
...
static void zreadrhs(int m, doublecomplex *b)
{
    ...
    for (i = 0; i < m; ++i)
      if(fscanf(fp, "%lf%lf\n", &b[i].r, &b[i].i)) printf("Error reading file");
    fclose(fp);
}
