...
static int ReadVector(FILE *fp, int n, int *where, int perline, int persize)
{
    ...
    i = 0;
    while (i < n) {
        if(fgets(buf, 100, fp)) printf("Error reading file");    /* read a line at a time */
        ...
        complex **nzval, int **rowind, int **colptr)
{
    ...
    /* Line 1 */
    if(fgets(buf, 100, fp)) printf("Error reading file");
    fputs(buf, stdout);

    /* Line 2 */
    for (i=0; i<4; i++) {
        if(fscanf(fp, "%14c", buf)) printf("error reading from file"); buf[14] = 0;
        sscanf(buf, "%d", &tmp);
    ...
    /* Line 3 */
    if(fscanf(fp, "%3c", type)) printf("Error reading file");
    if(fscanf(fp, "%11c", buf)) printf("Error reading file"); /* pad */
    type[3] = 0;
#ifdef DEBUG
    printf("Matrix type %s\n", type);
#endif

    if(fscanf(fp, "%14c", buf)) printf("Error reading file"); sscanf(buf, "%d", nrow);
    if(fscanf(fp, "%14c", buf)) printf("Error reading file"); sscanf(buf, "%d", ncol);
    if(fscanf(fp, "%14c", buf)) printf("Error reading file"); sscanf(buf, "%d", nonz);
    if(fscanf(fp, "%14c", buf)) printf("Error reading file"); sscanf(buf, "%d", &tmp);
    ...
    if(fscanf(fp, "%16c", buf)) printf("Error reading file");
    cParseIntFormat(buf, &colnum, &colsize);
    if(fscanf(fp, "%16c", buf)) printf("Error reading file");
    cParseIntFormat(buf, &rownum, &rowsize);
    if(fscanf(fp, "%20c", buf)) printf("Error reading file");
